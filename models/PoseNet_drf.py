...
def PoseNet_drf(data=None, nb_ref_imgs=2):
...
class pose_net(nn.Module):
    def __init__(self, nb_ref_imgs):
        ...
        self.conv1 = conv_elu(3 + 2, conv_planes[0])
    ...
    def encoder(self, input, flow):
        out_conv1 = self.conv1_1(self.conv1(torch.cat((input, flow), 1)))
    ...
    def forward(self, target_image, ref_imgs, intrinsics):
        ...
        flow = torch.ones(B, 2, H, W).type(target_image.type())
        flow[:, 0, :, :] = fx * flow[:, 0, :, :]
        flow[:, 1, :, :] = fy * flow[:, 1, :, :]

        # get target features
        tgt_feats = self.encoder(target_image, flow)

        # Get ref imgs feats
        ref_feats = []
        for i in range(len(ref_imgs)):
            ref_feats.append(self.encoder(ref_imgs[i], flow))
