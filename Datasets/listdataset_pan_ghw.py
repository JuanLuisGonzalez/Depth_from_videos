...
class ListDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.resize_crop_transform is not None:
            inputs, h, w, _ = self.resize_crop_transform(inputs, targets)
        ...
        # for grid
        grid = np.transpose(inputs[2], (2, 0, 1))
        grid = torch.from_numpy(grid.copy()).float()

        return inputs[0:2], [grid], x_pix, h, w, file_name
