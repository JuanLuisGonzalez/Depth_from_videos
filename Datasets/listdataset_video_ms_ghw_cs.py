...
def calib_loader(input_root, calib_path):
    ...
    intrinsics_c1[2, 2] = 1
    intrinsics_c1[0, 0] = calib_json['intrinsic']['fx']
    intrinsics_c1[1, 1] = calib_json['intrinsic']['fy']
    intrinsics_c1[0, 2] = calib_json['intrinsic']['u0'] - 100 # from cropping pre-processing
    intrinsics_c1[1, 2] = calib_json['intrinsic']['v0'] - 25  # from cropping pre-processing
...
class ListDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        inputs_l, inputs_r, calibration, targets = self.path_list[index]
        ...
        # Randomly pick continuous or skip one frame
        if random.random() < 0.5:
        ...
        cam_id = cam1_index
        intrinsics = calib_loader(self.input_root, calibration)
        # print(intrinsics)
        ign_mask = 0
